...
\[H_{N}(s) \approx -0,128 \cdot \frac{42,55 \cdot s}{s^2 + 42,55 \cdot s + 614,30^2} \cdot (-14,33) \cdot \frac{46,51 \cdot s}{s^2 + 46,51 \cdot s + 656,34^2}\]
...
\caption{Diagramas de bode de $H_{n}(s)$ (naranja) y $H(s)$ (azul)}
\end{figure}

Vemos que el error es aproximadamente $1dB$ (un poco menor). También podemos observar un ligero error en las frecuencias, esperable si consideramos que $W_{0_{1}} \approx 607,3\frac{r}{s}$ y nosotros obtuvimos un valor $W_{0_{1}} \approx 614,30\frac{r}{s}$ cuando normalizamos (lo mismo para $W_{0_{2}}$).
