...
la ganancia $H_{0}$ de forma tal de conseguir la correspondiente a la $H(s)$ asignada. Sin embargo tendremos un efecto secundario indeseado dado que  que, al ser inversor, nos correrá $180\degree$ la fase de la salida. Para contrarrestar esto utilizaremos un segundo amplificador inversor, cuyo factor será $k = -1$, con el objetivo de corregir la fase modificada por el anterior
...
Tomando $H_{0} = 1$ (utilizaremos amplificadores luego para conseguir la ganancia $H_{0}$ de nuestra función de transferencia) obtenemos finalmente el sistema

\[R_{2} = \frac{2 \cdot Q}{C \cdot W_{0}}\]
\[R_{1} = \frac{R_{2}}{2}\]
\[R_{3} = \frac{R_{1}}{2 \cdot Q^2 - 1}\]
...
de forma tal que tenemos dos filtros MFBP, por lo tanto tendremos que encontrar las constantes $C$, $R_{1}$, $R_{2}$ y $R_{3}$ para cada uno. Una vez que\\ hayamos obtenido dichos valores será cuestión de colocar ambos filtros MFBP en cascada para conseguir que las fracciones de $H(s)$ y por último utilizar amplificadores operacionales para conseguir $H_{0} = 2$.
